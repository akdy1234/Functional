...
(* ── Question 1.1 ────────────────────────────────────────────── *)
    ...
    (* HOW TO READ THE FUNCTION SYNTAX:
       ...
       Circle(r):
         Area = π × r².  Math.PI comes from the System namespace (opened at
         the top) and equals 3.14159265…  We write `r * r` rather than
         `r ** 2.0` to keep it explicit — both work in F#.

       Triangle(b, h):
         The exam says this is a RIGHT triangle (one 90-degree angle).
         Area of any triangle = (base × height) / 2.
         The 2.0 literal is needed because F# does not auto-promote int
         literals to float; writing just `2` here would be a type error. *)
    let area =
        function
        | Rectangle(w, h) -> w * h
    ...
    (* circumference / perimeter of each shape.
       ...
       Triangle(b, h):
         ...
         The third side (hypotenuse) has length √(b² + h²) by the
         Pythagorean theorem.
...
(* ── Question 1.3 ────────────────────────────────────────────── *)
    ...
    (* totalCircumference — tail-recursive via accumulator.
       ...
       (the running total).  Instead of leaving a pending addition on the
       stack, we UPDATE acc before the recursive call and pass the updated
       value in.  After the recursive call there is NOTHING left to do — it
       is the very last expression, so the runtime can reuse the current stack
       frame (this is what "tail call" means in practice).
       ...
       Notice the recursive call is the LAST thing in each step — no
       pending `+`. *)
...
(* ── Question 2.1 ────────────────────────────────────────────── *)
    ...
    A:
      foo : char -> char
        It takes one character and returns one character.  The body is
        `function | c -> ...` which matches a single value, and every
        branch returns a `char` (either `c` unchanged, or `char(...)`).

      bar : string -> char list
        The argument `(str : string)` is explicitly annotated as string.
        The body `[for c in str -> c]` is a list comprehension producing
        a list of chars (since iterating over a string yields chars).

      baz : string -> string
        Takes string `str`, converts it via bar to a char list, then
        `aux` processes that list and returns a string (the "" base case
    ...
    A:
      ...
      Step 2 — trace specific characters to spot the pattern:
        ...
        'y' (int = 121):  121 > 119                      → char(121-23)= char(98)  = 'b'
      ...
      Why -23 for x/y/z?  The alphabet has 26 letters.  Shifting forward by
      3 and wrapping is the same as shifting by -(26-3) = -23 for the last
      three letters.  char(int 'x' - 23) = char(120 - 23) = 'a'. ✓
      ...
      In F#, strings are sequences of chars, so iterating with `for c in str`
      visits each character left to right.
      ...
      - aux (char list) → recursively encodes the list:
          ...
          * c :: cs    → encode c with foo, convert to string with `string`,
...
(* ── Question 2.2 ────────────────────────────────────────────── *)
    ...
    (* WHY THE WARNING?
       ...
       has int value > 119 ('w'), or has int value ≤ 119 (i.e. < 120 = 'x').
